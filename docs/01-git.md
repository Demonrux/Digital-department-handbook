...
Чтобы отправить ваш код на GitHub необходимо создать там новый репозиторий (он будет удаленный) и после этого в терминале вашего локального проекта ввести команду:
```bash
git remote add <имя_репозитория> <URL_адрес>
```
 **имя_репозитория** это псевдоним вашего репозитория (короткое имя), чтобы при дальнейших отправках не писать полностью URL репозитория, обычно в качестве псевдонима использую origin
