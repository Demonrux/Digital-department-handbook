...
Чтобы отправить ваш код на GitHub необходимо создать там новый репозиторий (он будет удаленный) и связать его с вашим локальным, делается это командой:
```bash
...
Пример:
```bash
# Cвязал с репозиторием нашего справочника и задал псевдоним origin
...
Можно также поставить флаг *-u* (сокращение от --set-upstream)
